    ...
    <main className="mx-auto max-w-3xl px-6 py-16">
      ...
      <Section title="Add a slot">
        <form onSubmit={addSlot} className="flex flex-wrap gap-3 items-end">
          ...
          <div>
            <label className="block text-xs text-ink/50 mb-1">Length</label>
            <select
              value={duration}
            ...
            >
              ...
              <option value={180}>3 hrs — removal-friendly</option>
            </select>
          </div>
          <button
            type="submit"
        ...
        <p className="mt-3 text-xs text-ink/45">
          Mark a slot as 3 hrs if it's your last appointment that day, or if you're leaving
          room for a removal.
        </p>
      </Section>
        ...
        <ul className="divide-y divide-line/70 rounded-2xl ring-1 ring-line/70 overflow-hidden bg-white/40">
          {slots.map((s) => (
            <li key={s.id} className="flex items-center justify-between px-5 py-3 text-sm">
              <span>
                {s.date} · {s.time} ·{" "}
                <span className="text-ink/60">{(s.duration || 120) / 60} hrs</span>{" "}
            ...
            <li key={b.id} className="rounded-2xl ring-1 ring-line/70 p-4 bg-stoneDeep/40">
              <div className="flex items-start justify-between gap-4">
                <div className="text-sm">
                  ...
                  {b.instagram && <p className="text-ink/60">@{b.instagram.replace(/^@/, "")}</p>}
                  <p className="text-ink/60">
                    {b.date} · {b.time}
                    {b.duration ? ` (${b.duration / 60} hrs)` : ""}
                  </p>
                  <p className="text-ink/60">
                    {b.size} — {b.tier}
