"use client";

import { useEffect, useState } from "react";
import Nav from "@/components/Nav";
import Footer from "@/components/Footer";
import SwatchTier from "@/components/SwatchTier";
import { SIZES, TIERS, REMOVALS, priceFor } from "@/lib/pricing";

function formatDate(dateStr) {
  const d = new Date(`${dateStr}T00:00:00`);
  return d.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" });
}

function formatTime(timeStr) {
  const [h, m] = timeStr.split(":").map(Number);
  const d = new Date();
  d.setHours(h, m);
  return d.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
}

export default function Book() {
  const [slots, setSlots] = useState(null);
  const [slotId, setSlotId] = useState("");
  const [sizeId, setSizeId] = useState("");
  const [tierId, setTierId] = useState("");
  const [removalId, setRemovalId] = useState("");
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  const [email, setEmail] = useState("");
  const [status, setStatus] = useState("idle"); // idle | submitting | done | error
  const [errorMsg, setErrorMsg] = useState("");

  useEffect(() => {
    fetch("/api/slots")
      .then((r) => r.json())
      .then((data) => setSlots(data.slots || []))
      .catch(() => setSlots([]));
  }, []);

  const price = sizeId && tierId ? priceFor(sizeId, tierId, removalId || null) : null;
  const canSubmit = slotId && sizeId && tierId && name && phone;

  async function handleSubmit(e) {
    e.preventDefault();
    if (!canSubmit) return;
    setStatus("submitting");
    setErrorMsg("");
    try {
      const res = await fetch("/api/book", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slotId, sizeId, tierId, removalId: removalId || null, name, phone, email }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Something went wrong.");
      setStatus("done");
    } catch (err) {
      setStatus("error");
      setErrorMsg(err.message);
    }
  }

  if (status === "done") {
    return (
      <>
        <Nav />
        <main className="mx-auto max-w-xl px-6 py-28 text-center">
          <p className="text-xs uppercase tracking-[0.2em] text-umber mb-3">Request sent</p>
          <h1 className="font-display text-3xl text-inkDeep mb-4">
            You're on the list <span className="font-script text-4xl text-umber">✿</span>
          </h1>
          <p className="text-ink/70 mb-8">
            Send your $5 deposit via Zelle to{" "}
            <span className="text-inkDeep font-medium">626-295-8572</span> (no note needed — an
            emoji is fine if one's required). Your slot is confirmed once I approve your
            request.
          </p>
          <a
            href="https://instagram.com/natwnails"
            target="_blank"
            rel="noreferrer"
            className="text-umber hover:underline"
          >
            Questions? DM @natwnails
          </a>
        </main>
        <Footer />
      </>
    );
  }

  return (
    <>
      <Nav />
      <main className="mx-auto max-w-2xl px-6 pt-16 pb-24">
        <p className="text-xs uppercase tracking-[0.2em] text-umber mb-3">Book a slot</p>
        <h1 className="font-display text-4xl text-inkDeep mb-4">
          Pick your <span className="font-script text-5xl text-umber">appointment</span>
        </h1>
        <p className="text-ink/70 mb-10">
          Slots are posted monthly and go fast. Choose an open time, your service, and confirm
          with a $5 deposit.
        </p>

        <form onSubmit={handleSubmit} className="space-y-10">
          {/* Slots */}
          <div>
            <h2 className="font-display text-lg italic text-inkDeep mb-4">1. Open slots</h2>
            {slots === null && <p className="text-sm text-ink/50">Loading availability…</p>}
            {slots !== null && slots.length === 0 && (
              <p className="text-sm text-ink/60 rounded-xl bg-stoneDeep/60 ring-1 ring-line/70 p-4">
                No open slots right now — check back soon or follow @natwnails for when new
                dates drop.
              </p>
            )}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {slots?.map((s) => (
                <button
                  type="button"
                  key={s.id}
                  onClick={() => setSlotId(s.id)}
                  className={`rounded-xl px-4 py-3 text-left text-sm ring-1 transition ${
                    slotId === s.id
                      ? "bg-inkDeep text-mist ring-inkDeep"
                      : "ring-line hover:bg-mist text-ink"
                  }`}
                >
                  <span className="block font-medium">{formatDate(s.date)}</span>
                  <span className="block opacity-80">{formatTime(s.time)}</span>
                </button>
              ))}
            </div>
          </div>

          {/* Size */}
          <div>
            <h2 className="font-display text-lg italic text-inkDeep mb-4">2. Length</h2>
            <div className="grid gap-2">
              {SIZES.map((s) => (
                <button
                  type="button"
                  key={s.id}
                  onClick={() => setSizeId(s.id)}
                  className={`flex items-center justify-between rounded-xl px-4 py-3 text-left text-sm ring-1 transition ${
                    sizeId === s.id ? "bg-mist ring-inkDeep" : "ring-line hover:bg-mist"
                  }`}
                >
                  <span>{s.label}</span>
                  <span className="text-umber font-display">${s.price}</span>
                </button>
              ))}
            </div>
          </div>

          {/* Tier */}
          <div>
            <h2 className="font-display text-lg italic text-inkDeep mb-4">3. Design tier</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {TIERS.map((tier) => (
                <SwatchTier
                  key={tier.id}
                  tier={tier}
                  interactive
                  selected={tierId === tier.id}
                  onClick={() => setTierId(tier.id)}
                />
              ))}
            </div>
          </div>

          {/* Removal */}
          <div>
            <h2 className="font-display text-lg italic text-inkDeep mb-4">
              4. Removal <span className="text-sm text-ink/50 font-body not-italic">(if needed)</span>
            </h2>
            <div className="grid gap-2 sm:grid-cols-3">
              <button
                type="button"
                onClick={() => setRemovalId("")}
                className={`rounded-xl px-4 py-3 text-left text-sm ring-1 transition ${
                  removalId === "" ? "bg-mist ring-inkDeep" : "ring-line hover:bg-mist"
                }`}
              >
                None needed
              </button>
              {REMOVALS.map((r) => (
                <button
                  type="button"
                  key={r.id}
                  onClick={() => setRemovalId(r.id)}
                  className={`flex items-center justify-between rounded-xl px-4 py-3 text-left text-sm ring-1 transition ${
                    removalId === r.id ? "bg-mist ring-inkDeep" : "ring-line hover:bg-mist"
                  }`}
                >
                  <span>{r.label}</span>
                  <span className="text-umber font-display">+${r.price}</span>
                </button>
              ))}
            </div>
          </div>

          {/* Contact */}
          <div>
            <h2 className="font-display text-lg italic text-inkDeep mb-4">5. Your info</h2>
            <div className="grid gap-3">
              <input
                required
                placeholder="Full name"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="rounded-xl px-4 py-3 bg-mist ring-1 ring-line focus:ring-inkDeep outline-none text-sm"
              />
              <input
                required
                type="tel"
                placeholder="Phone number"
                value={phone}
                onChange={(e) => setPhone(e.target.value)}
                className="rounded-xl px-4 py-3 bg-mist ring-1 ring-line focus:ring-inkDeep outline-none text-sm"
              />
              <input
                type="email"
                placeholder="Email (optional)"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="rounded-xl px-4 py-3 bg-mist ring-1 ring-line focus:ring-inkDeep outline-none text-sm"
              />
            </div>
          </div>

          {/* Summary + submit */}
          <div className="rounded-2xl bg-stoneDeep/60 ring-1 ring-line/70 p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-ink/70">Total due at appointment</span>
              <span className="font-display text-2xl text-umber">
                {price != null ? `$${price}` : "—"}
              </span>
            </div>
            <p className="text-xs text-ink/50 mb-4">
              A $5 Zelle deposit (626-295-8572) secures this slot once your request is approved.
            </p>
            {errorMsg && <p className="text-sm text-umber mb-4">{errorMsg}</p>}
            <button
              type="submit"
              disabled={!canSubmit || status === "submitting"}
              className="w-full rounded-full bg-inkDeep px-7 py-3 font-body text-mist transition hover:bg-umber disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {status === "submitting" ? "Sending request…" : "Request this slot"}
            </button>
          </div>
        </form>
      </main>
      <Footer />
    </>
  );
}
