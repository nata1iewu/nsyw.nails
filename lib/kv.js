...
export async function addBooking(booking) {
  ...
  const newBooking = {
    ...
    manageToken: booking.manageToken || crypto.randomBytes(16).toString("hex"),
  };
...
export async function setBookingSlot(id, newSlotId, date, time) {
  if (!redis) return;
  const bookings = (await redis.get("natwnails:bookings")) || [];
  const updated = bookings.map((b) =>
    b.id === id ? { ...b, slotId: newSlotId, date, time, status: "pending" } : b
  );
  await redis.set("natwnails:bookings", updated);
}
export async function setBookingReminderSent(id) {
  if (!redis) return;
  const bookings = (await redis.get("natwnails:bookings")) || [];
  const updated = bookings.map((b) => (b.id === id ? { ...b, reminderSent: true } : b));
  await redis.set("natwnails:bookings", updated);
}