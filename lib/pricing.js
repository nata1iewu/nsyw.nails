export const SIZES = [
  { id: "natural", label: "Natural nails", price: 40, nonStudentPrice: 55 },
  { id: "short", label: "X-Short / Short extensions (Gel-X)", price: 45, nonStudentPrice: 60 },
  { id: "medium", label: "Medium extensions (Gel-X)", price: 50, nonStudentPrice: 65 },
  { id: "long", label: "Long extensions (Gel-X)", price: 55, nonStudentPrice: 70 },
  { id: "xlong", label: "XL extensions (Gel-X)", price: 60, nonStudentPrice: 75 },
];

export const REMOVALS = [
  { id: "with_set", label: "Removal with a new set", price: 10 },
  { id: "without_set", label: "Removal without a new set", price: 15 },
];
...
export const TIERS = [
  ...
  },
  {
    id: "tier2",
    label: "Two",
    desc: "Slightly elevated but still simple designs",
    add: 5,
  ...
  },
  {
    id: "tier3",
    label: "Three",
    desc: "Detailed design with multiple elements",
    add: 10,
...
export function priceFor(sizeId, tierId, removalId = null, isStudent = true) {
  const size = SIZES.find((s) => s.id === sizeId);
  const tier = TIERS.find((t) => t.id === tierId);
  if (!size || !tier) return null;
  const removal = removalId ? REMOVALS.find((r) => r.id === removalId) : null;
  const basePrice = isStudent ? size.price : size.nonStudentPrice;
  return basePrice + tier.add + (removal ? removal.price : 0);
}